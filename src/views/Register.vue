    ...
    <div class="p-6 max-w-sm w-full bg-white shadow-md rounded-md">
      ...
      <form class="mt-4" @submit.prevent="handleSubmit">
        ...
        <label class="block mt-3">
          ...
          />
          <div v-show="submitted && !user.confirmPassword">Password is required</div>
        </label>
...
export default {
  ...
  methods: {
    handleSubmit(e) {
      this.submitted = true;
      const { username, password, first_name, last_name } = this.user;
      if (first_name && last_name && username && password) {
        if (this.user.confirmPassword === password) {
